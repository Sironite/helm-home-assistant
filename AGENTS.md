...
Helm chart for deploying [Home Assistant](https://www.home-assistant.io) on Kubernetes.
...
helm template home-assistant . -f values.yaml.example
...
- LSIO s6-overlay images are incompatible with `readOnlyRootFilesystem: true` → KSV014 suppressed in `.trivyignore`.
- `hostNetwork: true` auto-sets namespace PSS label to `privileged`.
